...
namespace System {
    ...
    public partial class Uri : ISerializable {
            ...
            get {
                ...
                if (segments == null) {
                    ...
                    if (path.Length == 0) {
                        ...
                        Collections.Generic.List<string> pathSegments = new Collections.Generic.List<string>();
                        int current = 0;
                        ...
                        segments = (string[])(pathSegments.ToArray());
                    }
                }
                return segments;
            }
        ...
        private static volatile UriIdnScope s_IdnScope = System.UriIdnScope.None;

        // Value from config Uri section
        // On by default in .NET 4.5+ and cannot be disabled by config.
        private static volatile bool s_IriParsing =
            (UriParser.ShouldUseLegacyV2Quirks ? false : true);
            ...
            else
            {
                for (int idx=0; idx < portStr.Length; ++idx)
                {
                    ...
                    if (val < 0 || val > 9 || (port = (port * 10 + val)) > 0xFFFF)
                        throw new UriFormatException(SR.Format(SR.net_uri_PortOutOfRange, m_Syntax.GetType().FullName, portStr));
                }
